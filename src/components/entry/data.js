...
let entries = [
    ...
    {key:nanoid(), text:`It's useful to split contamination into passive and active factors. Passive factors stay the same during the welding process, like surface prep, cleaning, gas and welder settings`},
    {key:nanoid(), text:`Surface prep and cleaning show up as poorly starting arc or improper arc color. Gas flow around 12CFH is fine and usually is not as sensitive as other factors. Several times I have had too little amperage and went on troubleshooting incorrectly, because it forms a poor arc that looks like contamination. Also the TIG arc is a little more fragile to wind and magnetism than MIG or Stick. The fan from my unit (under the table) was enough to disturb the arc, and often using holding magnets also causes the arc to deflect. `},
    {key:nanoid(), text:`Once you get a little bit of contamination the weld quality spirals downward as oxide penetrates the weld and contaminates deep into the metal, making a steel blister that signals more grinding in your future. A shiny and clean surface welds best, but there is a little leeway.`},
    {key: 'tig-active-contamination', subtitle:`TIG Control` },
    {key:nanoid(), text:`Active factors are ones that must be controlled while the weld is underway. Get a nice pool developed and dip the rod in. Then you develop the muscle memory to advance the arc, feed the filler in the off hand near but not touching the puddle, getting the pool to size and then dipping once again. On straightaways it's simple but it can be difficult as your body contorts to different positions as the weld moves. It's good to do a little air welding to practice the positions before lighting the arc.`},
    {key:nanoid(), text:`A helmet with good visibility is key. Although in many welding examples online use a big macro lens, the weld is not that big! Especially with a small 3/32" electrode the weld is 5mm across. The torch has to be controlled to under a millimeter with various different movements, in different directions and poses. Not being able to see makes everything harder.`},
    {key: 'thin-metals', subtitle:`Thin Metals` },
    {key:nanoid(), text:`TIG welding is especially suited for thin metals, because the heat can be controlled easier and requires less amperage than MIG. While all previous TIG advice applies, be especially careful to keep the puddle moving, maintain a short arc to reduce heat buildup, and if possible use a backer metal which greatly reduces heat warpage, protects from burn through and gives you more time and control over the weld. Pulsing also helps keep the puddle moving as well as forming a natural rhythm. `},
...
`
    ...
    {key:nanoid(), img:'addlathe5', style: {gridColumn:"span 5", marginBottom: "30px", height:"300px"}, className:"entry--img-inline", imageName:'./addlathe5.png'},
    ...
    {key:nanoid(), img:'addlathe2', style: {gridColumn:"span 5", marginBottom: "30px", height:"300px"}, className:"entry--img-inline", imageName:'./addlathe2.jpg'},
    ...
    {key:nanoid(), img:'addlathe4', style: {gridColumn:"span 5", marginBottom: "30px", height:"440px"}, className:"entry--img-inline", imageName:'./addlathe4.jpg'},
    ...
    {key:5, img:'mpcnc2', style: {gridColumn:"span 3", gridRow:"span 3", marginBottom:"30px"}, className:"entry--img-inline",  imageName:'./mpcnc2.jpg'},
    ...
    {key:nanoid(), html:<div>Start with a simple plywood box with two rabbetted pieces and two intact ones. Add a slot in the bottom. Conveniently, a 1/8" blade fits the thin 1/8" hardboard suitable for drawer bottoms perfectly. Now you have a box. </div>},
    {key:nanoid(), html:<div>You want a drawer face? Well, change one side to dadoes to cover one side completely. Make a big box to fit it in. All the same tricks, but bigger. Put 5 drawers in a box. Now you need to do a little writing before you cut too many things. Put the drawers in the box. Now you've got one less drawer to make.</div>},
    ...
    {key:nanoid(), img:'mitre-sled', style: {gridColumn:"1 / span 2", gridRow:"span 2",  marginBlock:"20px", height:'40vh'}, className:"entry--img-inline", imageName:'./wood3.jpg' },
    ...
    {key:nanoid(), img:'cross-cut-sled', style: {gridColumn:"4/ span 2", gridRow:"span 4", marginBottom:"20px",}, className:"entry--img-inline", imageName: './wood13.jpg'},
    {key: 'chessboard', title:`Chessboard`},
    {key:nanoid(), img:'chessboard', style: {gridColumn:"span 2", gridRow:"span 4", marginBottom:"20px",}, className:"entry--img-inline", imageName: './wood2.jpg'},
    ...
    {key:nanoid(), img:'wood-1', style: {gridColumn:"span 3",  marginBlock:"20px", height:'40vh'}, className:"entry--img-inline", imageName:'./wood1.jpg' },
    {key:nanoid(), img:'wood-5', style: {gridColumn:"1 /span 2",  marginBottom:"20px", height:'40vh'}, className:"entry--img-inline", imageName:'./wood5.jpg' },
    {key:nanoid(), img:'wood-9', style: {gridColumn:"span 3",  marginBottom:"20px", height:'40vh'}, className:"entry--img-inline", imageName:'./wood9.jpg' },
    ],
    ...
    {key:nanoid(), html:<>One of the great joys of climbing is jangling your gear around. I tend to go with a harness with sling PAS, <b>sport anchor</b>, cleaning carabiner, <b>2 alpine draws</b> and my long reach draw. My belay device, gloves and glasses are on one carabiner. I clip my climbing shoes together and attach them to my harness while I'm belaying, so it's easy to switch to climbing mode. Add draws or other gear as needed.</>, style: {gridColumn:"1 /span 3"}},
    {key:nanoid(), img:'climbing-gear', className:"entry--img-inline", style:{gridColumn:"4 / span 2", marginBottom:"0px"},picStyle: {objectFit: "cover", height:"180px",  objectPosition:"60% 60%" },  imageName:'./climbing-6.jpg' },
    {key:nanoid(), },
    ...
    {key:nanoid(), subtitle: `On Sport Anchors`},
    {key:nanoid(), html:<>On the internet trivia outperforms principles. Sport anchors are not trad anchors and the requirements are two points and each is bomber. Therefore, chains with a single locker, or two quickdraws is an effective anchor. But, let's say you want it equalized just because you like it..</>},
    {key:nanoid(), html:<>BFK, Limited sliding X, and Quad<br></br>Although simple, these anchors tend to cinch the knots tighter and tighter and become a pain to undo after a few uses. Friendly reminder that knots reduce the strength of slings by 50%! But that's still super good enough.</>},
    {key:nanoid(), html:<>Swamp, bunny ears<br></br>Other exotic anchors that provide good attachment but need to be tied or adjusted at the anchor which reduces the usefulness for sport climbing</>},
    {key:nanoid(), html:<>Girth hitch masterpoint, rappel ring<br></br>Now here's an anchor that's easy to adjust, equalized, and lightweight. The only issue is if you clip the rappel ring (or carabiner) on the wrong side it flips and undoes the girth hitch. This failure mode must disqualify it.</>},
    {key:nanoid(), html:<>Girth X, girth hitch with a sliding X<br></br> Removes the flip failure mode by clipping the inside of the loop, but the girth hitch gets quite bulky and hard to adjust. Contender but not finalist for best anchor.</>},
    {key:nanoid(), html:<>Sliding X<br></br> Pure sliding X with no limiters. Great anchor, gets full strength and shock loading doesn't occur since your body is a bag of water. Finalist.</>},
    {key:nanoid(), html:<>Girth Sliding X<br></br> Doubled over 120cm sling with a girth hitch on one side. Pretied works mostly, but it's also easily adjustable and there are two separate sliding X's to clip. My go to anchor, the knot doesn't get locked and the girth hitch doesn't slip under load. A minute improvement can be made by using two separate 120cm slings.. but the tacks are getting bulky and I could use 2 alpine draws.</>},
    {key:nanoid(), html:<></>},
    ...
    {key:nanoid(), img:'3dart-2', className:"entry--img-inline", style:{gridColumn:"4 / span 2", marginBottom:"20px"},picStyle: {objectFit: "contain", height:"auto" },  imageName:'./3dart-2.png' },
    ...
    {key:nanoid(), img:'model-splash-2', className:"entry--img-inline", style:{gridColumn:"1 / span 5", marginBottom:"20px"},picStyle: {objectFit: "contain", height:"auto" },  imageName:'./model-splash-2.png' },
    ...
    {key:nanoid(), img:'3dart-4', className:"entry--img-inline", style:{gridColumn:"4 / span 2", marginBottom:"20px"},picStyle: {objectFit: "contain", height:"auto" },  imageName:'./3dart-4.png' },
