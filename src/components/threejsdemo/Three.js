...
const threes = (props) => {
    const [modelShown, setModelShown] =  React.useState(0);
    const [zoom, setZoom] = React.useState(props.zoom);

    function loadThreeMF(loader, modelData, list, modelShown, scene) {
    ...
    const camera = new THREE.OrthographicCamera(width / -2, width / 2, height / 2, height / -2, 0, 2000)//( 10, width / height, 50, 800)
    // camera = new THREE.PerspectiveCamera( 5, width / height, 50, 2000)
    camera.zoom = zoom
    camera.up.set(0, 0, 1);
...
//             modelShown > 0 && this.changeModelShown(modelShown - 1)}>Prev</Button> {' '}
//           <Button variant="primary" size='lg' className={'threescene--button'} onClick={() =>
//             modelShown < modelList.length - 1 && this.changeModelShown(modelShown + 1)}>Next Model</Button>
//         </div>
